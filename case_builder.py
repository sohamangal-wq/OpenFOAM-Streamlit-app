...
class CaseBuilder:
    ...
    def __init__(self, base_template):
        ...
        if not os.path.exists(self.template_path):
            print(f"Template missing at {self.template_path}. Auto-generating base template...")
            setup_template_v13_multicomponent.create_base_template()
    ...
    def get_stl_bounds(self, stl_bytes):
        min_bounds = [float('inf')] * 3
        max_bounds = [float('-inf')] * 3
        
        try:
            if len(stl_bytes) >= 84:
                num_faces = struct.unpack('<I', stl_bytes[80:84])[0]
                expected_size = 84 + num_faces * 50
                if len(stl_bytes) == expected_size:
                    for i in range(num_faces):
                        offset = 84 + i * 50 + 12
                        for j in range(3):
                            v_offset = offset + j * 12
                            v = struct.unpack('<3f', stl_bytes[v_offset:v_offset+12])
                            for k in range(3):
                                if v[k] < min_bounds[k]: min_bounds[k] = v[k]
                                if v[k] > max_bounds[k]: max_bounds[k] = v[k]
        ...
        try:
            ...
            for v in vertices:
                for i in range(3):
                    val = float(v[i])
                    if val < min_bounds[i]: min_bounds[i] = val
                    if val > max_bounds[i]: max_bounds[i] = val
            if min_bounds[0] != float('inf'):
                return min_bounds, max_bounds
        except: pass
    ...
    def _write_topoSetDict(self, case_path, in_center, out_center, area_sqft, sim_mode):
        header = """/*--------------------------------*- C++ -*----------------------------------*\\
| =========                 |                                                 |
...
\\*---------------------------------------------------------------------------*/\n"""
        ...
        if sim_mode == "Closed Room CFD" and in_center and out_center:
            area_m2 = area_sqft * 0.092903
            side_len = math.sqrt(area_m2)
            box_size = (side_len / 2.0) + 0.02 
            topo_content += f"""
...
"""
    ...
    def create_case_from_group(self, case_name, stl_file, leak_group_df, mesh_res="Fast", refine_lvl=1, num_cores=1,
        ...
        if os.path.exists(zero_dir):
            valid_base_files = ["U", "p", "p_rgh", "T", "k", "epsilon", "omega", "nut", "alphat"]
        ...
        b_min, b_max = [-5, -5, -5], [5, 5, 5]
        if stl_file:
            try:
                if hasattr(stl_file, "getvalue"):
                    stl_data = stl_file.getvalue()
                else:
                    stl_file.seek(0)
                ...
                with open(os.path.join(tri_path, "room.stl"), "wb") as f:
                    f.write(stl_data)
                b_min, b_max = self.get_stl_bounds(stl_data)
            except Exception as e:
                with open(os.path.join(tri_path, "room.stl"), "wb") as f:
                    shutil.copyfileobj(stl_file, f)
                    
            pad = 2.0
            b_min = [x - pad for x in b_min]
            b_max = [x + pad for x in b_max]
        ...
        for index, row in leak_group_df.iterrows():
            ...
            try:
                ...
                for p in pairs:
                    ...
                    if spec in ["H2", "He"]: is_light_gas_case = True
            except:
                components = [("CH4", 1.0)]
        ...
        cfg_dict = {"inlet": vent_inlet_center, "outlet": vent_outlet_center, "leak": leak_loc}
        with open(os.path.join(case_path, "case_config.json"), "w") as f:
            json.dump(cfg_dict, f)

        actual_max_co = max_courant
        if sim_mode == "Closed Room CFD" and mesh_res == "Fast":
            actual_max_co = max(2.0, max_courant)

        self._rewrite_controlDict(
            case_path, self.vent_inlet, self.vent_outlet, list(all_scenario_species),
            sim_mode=sim_mode, mode=mesh_res, is_light_gas=is_light_gas_case, probe_locations=probe_locations,
            sim_duration=sim_duration, max_courant=actual_max_co, delta_t=delta_t,
            primary_gas=primary_gas, leak_loc=leak_loc, b_min=b_min, b_max=b_max
        )

        self._edit_thermo_multi(case_path, list(all_scenario_species))
        self._edit_thermophysicalTransport(case_path)
        self._edit_combustionProperties(case_path)
        self._edit_fvSolution(case_path, mode=mesh_res, is_light_gas=is_light_gas_case)

        centroid = [(b_max[0]+b_min[0])/2.0, (b_max[1]+b_min[1])/2.0, (b_max[2]+b_min[2])/2.0]

        if sim_mode == "Closed Room CFD":
            self._edit_blockMesh_split(case_path, mesh_res, b_min, b_max)
        else:
            self._edit_blockMesh_outdoor(case_path, mesh_res, b_min, b_max)

        self._write_topoSetDict(case_path, vent_inlet_center, vent_outlet_center, hvac_area_sqft, sim_mode)

        first_leak_inj = [{'x': list(injections.values())[0]['x'], 'y': list(injections.values())[0]['y'], 'z': list(injections.values())[0]['z']}] if injections else []
        self._edit_snappyHexMesh(case_path, first_leak_inj, refine_lvl, centroid, mode=mesh_res)
    ...
    def _edit_thermophysicalTransport(self, case_path):
        path = os.path.join(case_path, "constant", "thermophysicalTransport")
        content = """/*--------------------------------*- C++ -*----------------------------------*\\
| =========                 |                                                 |
| \\\\      /  F ield         | OpenFOAM: The Open Source CFD Toolbox           |
|  \\\\    /   O peration     | Version:  v13                                   |
...
"""
    ...
    def _edit_combustionProperties(self, case_path):
        path = os.path.join(case_path, "constant", "combustionProperties")
        content = """/*--------------------------------*- C++ -*----------------------------------*\\
| =========                 |                                                 |
| \\\\      /  F ield         | OpenFOAM: The Open Source CFD Toolbox           |
|  \\\\    /   O peration     | Version:  v13                                   |
...
{
"""
        # PERFECT PHYSICS FIX: A positive flowRateInletVelocity blows INTO the room. 
        # (Negative turns the vent into a vacuum exhaust, dropping the outlet concentration to zero)
        if sim_mode == "Closed Room CFD":
...
"""
    ...
    ".*" {{ type epsilonWallFunction; value uniform 0.01; }}
        ...
        omega_content = header + """FoamFile { version 2.0; format ascii; class volScalarField; object omega; }
...
"""
        if sim_mode == "Closed Room CFD":
            omega_content += f"""    {self.vent_inlet} {{ type fixedValue; value uniform 0.01; }}
    {self.vent_outlet} {{ type inletOutlet; inletValue uniform 0.01; value uniform 0.01; }}
    ".*" {{ type omegaWallFunction; value uniform 0.01; }}
}}"""
        else:
            omega_content += f"""    inlet {{ type atmBoundaryLayerInletOmega; flowDir ({dir_x} {dir_y} 0); zDir (0 0 1); Uref {wind_speed}; Zref 10.0; z0 uniform {z0}; zGround uniform 0.0; value uniform 0.01; }}
    outlet {{ type inletOutlet; inletValue uniform 0.01; value uniform 0.01; }}
    sky {{ type inletOutlet; inletValue uniform 0.01; value uniform 0.01; }}
    ground {{ type omegaWallFunction; value uniform 0.01; }}
    ".*" {{ type omegaWallFunction; value uniform 0.01; }}
}}"""
        with open(os.path.join(case_path, "0", "omega"), "w") as f: f.write(omega_content)
...
"""
    ...
    def _edit_blockMesh_outdoor(self, case_path, res, b_min, b_max):
...
vertices
(
    ({b_min[0]} {b_min[1]} {b_min[2]}) ( {b_max[0]} {b_min[1]} {b_min[2]}) ( {b_max[0]} {b_max[1]} {b_min[2]}) ({b_min[0]} {b_max[1]} {b_min[2]})
    ({b_min[0]} {b_min[1]} {b_max[2]}) ( {b_max[0]} {b_min[1]} {b_max[2]}) ( {b_max[0]} {b_max[1]} {b_max[2]}) ({b_min[0]} {b_max[1]} {b_max[2]})
);
...
"""
    ...
    def _edit_fvSolution(self, case_path, mode="Fast", is_light_gas=False):
        outer_corr = "1"
        inner_corr = "2" if is_light_gas else "1"
        path = os.path.join(case_path, "system", "fvSolution")
        if os.path.exists(path):
            with open(path, 'r') as f: content = f.read()
            content = content.replace("nOuterCorrectors 3;", f"nOuterCorrectors {outer_corr};")
            content = content.replace("nCorrectors 1;", f"nCorrectors {inner_corr};")
            if is_light_gas:
                content = content.replace("nCellsInCoarsestLevel 20;", "nCellsInCoarsestLevel 10;")
            with open(path, 'w') as f: f.write(content)

    def _edit_blockMesh_split(self, case_path, res, b_min, b_max):
        cells = 25 if res == "Fast" else 45
        block_mesh_content = f"""/*--------------------------------*- C++ -*----------------------------------*\\
| =========                 |                                                 |
| \\\\      /  F ield         | OpenFOAM: The Open Source CFD Toolbox           |
|  \\\\    /   O peration     | Version:  v13                                   |
...
"""
    ...
    def _write_topoSet_createPatch_subset(self, case_path, in_center, out_center, area_sqft):
        area_m2 = area_sqft * 0.092903
        side_len = math.sqrt(area_m2)
        box_size = (side_len / 2.0) + 0.02 
...
"""
    ...
    def _edit_snappyHexMesh(self, case_path, injections, refine_lvl, centroid, mode="Fast"):
        surface_lvl = "(1 1)" if mode == "Fast" else "(3 3)"
        region_lvl = "1" if mode == "Fast" else str(max(1, refine_lvl))
        centroid_str = f"({centroid[0]} {centroid[1]} {centroid[2]})"
        ...
        if injections:
            ...
            refine_box_str = f"refineBox {{ type searchableBox; min ({box_min}); max ({box_max}); }}"
            refine_region_str = f"refineRegions {{ refineBox {{ mode inside; levels ((1.0 {region_lvl})); }} }}"
...
{{
    ...
    refinementSurfaces {{ room {{ level {surface_lvl}; patchInfo {{ type wall; name room; }} }} }}
    {refine_region_str}

    resolveFeatureAngle 30; insidePoint {centroid_str};
    maxLocalCells 500000; maxGlobalCells 500000; minRefinementCells 10;
...
mergeTolerance 1e-6;
"""
        with open(os.path.join(case_path, "system", "snappyHexMeshDict"), "w") as f:
            f.write(shm_content)
...
"""
        for i, inj in enumerate(injections):
            ...
            # CRITICAL FPE FIX: Reverted strictly to stable 'points' injection (bypasses cellZone mesh failure)
            options_content += f"""
...
"""
    ...
    def _rewrite_controlDict(self, case_path, inlet_name, outlet_name, species_list,
                             sim_mode="Closed Room CFD", mode="Fast", is_light_gas=False, probe_locations=None, sim_duration=200, max_courant=1.0,
                             delta_t=0.0001, primary_gas="CH4", leak_loc=(0,0,0), b_min=[-5,-5,-5], b_max=[5,5,5]):
...
"""
        ...
        if not probe_locations:
            probe_locations = []
        probe_locations.append([lx, ly, lz + 1.0])
...
"""
        ...
        max_dt = 0.1 if is_light_gas else 0.05
        default_max_co = 1.0
        if sim_mode == "Closed Room CFD" and mode == "Fast":
            default_max_co = max_courant
...
adjustTimeStep yes;
maxCo {default_max_co};
maxAlphaCo {default_max_co};
maxDeltaT {max_dt};
...
{field_avg_str}
{surfaces_str}
{streamlines_hvac_str}
{streamlines_leak_str}
}}
"""
